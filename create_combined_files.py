...
machine_json_paths = { x.stem: x for x in Path('system/BBL').glob("**/*.json") if 'combined' not in x.stem}
# filament_json_files = { x.stem: x for x in  Path('system/BBL/machine').glob("**/*.json")}
# process_json_files = { x.stem: x for x in  Path('system/BBL/machine').glob("**/*.json")}

for machine_json_path in machine_json_paths.values():
    with open(machine_json_path) as machine_json_file:
        logger.debug(f'Loading "{machine_json_path}" as json')
        try:
            machine_json_content = json.load(machine_json_file)
            fully_loaded_config = load_inherited_attributes(machine_json_content, machine_json_path.stem)
            ...
            if 'inherits' in fully_loaded_config:
                combined_file_name = machine_json_path.parent.joinpath('combined_' + machine_json_path.stem.lower().replace(' ', '_') +  machine_json_path.suffix)
                ...
                logger.debug(f'Skipping output for parent machine: {machine_json_path.stem}')
        except JSONDecodeError as e:
            logger.error(f'Could not load "{machine_json_path}" as json: {e}')
            continue
